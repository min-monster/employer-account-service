...
final case class AccountRepositoryImpl(xa: Transactor[Task], console: Console.Service)
  ...
  def getAllAccounts: Task[List[Account]] =
    SQL
      .getAllAccounts()
      .transact(xa)
      .foldM(err => Task.fail(err), output => Task.succeed(output))
    ...
    getAccount(account.id)
      ...
      .orElse(
        SQL
          ...
          .transact(xa)
          .foldM(err => Task.fail(err), _ => Task.succeed(account))
      )
  ...
  def updateAccount(account: Account): Task[Boolean] =
    getAccount(account.id)
      .flatMap(
        account1 =>
          Task
            .succeed(println(s"Updating account ${account1} to ${account}")) *>
            SQL
              .updateAccount(account)
              .transact(xa)
              .fold(_ => false, _ => true)
      )
      .orElse(Task.fail(AccountNotFound(account.id)))

  object SQL {

    import dc._

    // sql"""SELECT id, name, contact_id, zuora_id, crm_id, website,
    // parent_account_id, address FROM
    // ACCOUNT""".queryWithLogHandler[Account](LogHandler.jdkLogHandler)
    def getAllAccounts() =
      run(quote {
        account
      })
    ...
    // sql"""SELECT id, name, contact_id, zuora_id, crm_id, website,
    // parent_account_id, address FROM ACCOUNT where
    // id=${id}""".queryWithLogHandler[Account](LogHandler.jdkLogHandler)
    def getAccount(id: String) =
      run(quote {
        account.filter(_.id == lift(id))
      })

    // sql"""INSERT INTO ACCOUNT (id, name, contact_id, zuora_id, crm_id,
    // website, parent_account_id, address) VALUES
    // (${account.id}, ${account.name}, ${account.contactId},
    // ${account.zuoraId}, ${account.crmId}, ${account.website},
    // ${account.parentAccountId}, ${account.address})
    ...
    // sql"""UPDATE ACCOUNT SET name = ${account.name} WHERE id =
    // ${account.id}""".updateWithLogHandler(LogHandler.jdkLogHandler)
    def updateAccount(acc: Account) =
      run(quote {
