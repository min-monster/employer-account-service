...
final case class UserRepositoryImpl(xa: HikariTransactor[Task]) extends UserService {
  ...
  def create(user: User): Task[User] =
    get(user.id)
      .flatMap(user => Task.succeed(println("There is a duplicate, so not inserting")) *> Task.succeed(user))
      .orElse(
        SQL
          .create(user)
          .transact(xa)
  ...
  def update(user: User): Task[Boolean] =
    get(user.id)
      .flatMap(
        user1 =>
          Task.succeed(println(s"Updating user ${user1} to ${user}")) *>
            SQL
              .update(user)
              .transact(xa)
              .fold(_ => false, _ => true)
...
object SQL {
  ...
  // USER""".queryWithLogHandler[User](LogHandler.jdkLogHandler)
  def getAll =
    run(quote {
      user
    })
  ...
  // id=${id}""".queryWithLogHandler[User](LogHandler.jdkLogHandler)
  def get(id: Long) =
    run(quote {
      user.filter(_.id == lift(id))
    })

  // sql"""INSERT INTO USER (id, name) VALUES (${user.id},
  // ${user.name})""".updateWithLogHandler(LogHandler.jdkLogHandler)
  def create(userIn: User) =
    run(quote {
  ...
  // sql"""UPDATE USER SET name = ${user.name} WHERE id =
  // ${user.id}""".updateWithLogHandler(LogHandler.jdkLogHandler)
  def update(userIn: User) =
    run(quote {
